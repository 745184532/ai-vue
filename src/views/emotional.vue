<template>
  <div>
    <PageHead title="情绪日志" />
    <TableSearch :fromItem="formItem" @search="handleSearch" />
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column label="操作" width="240" fixed="right">
        <template #default="scope">
          <el-button @click="viewDetailDialog(scope.row)" text type="primary">详情</el-button>
    ...
    <el-dialog v-model="detailDialogVisible" title="情绪日志详情" width="800px" :close-on-click-modal="false">
      <div class="detail-content" v-if="currentDetail">
        ...
        <div class="detail-section">
          <h4>情绪状态</h4>
          <el-descriptions :column="2" border>
            <el-descriptions-item label="情绪频分">
        ...
        <div class="detail-section">
          <h4>时间信息</h4>
          <el-descriptions :column="2" border>t
...
import { getEmotionalScore,deleteEmotional } from '@/api/admin.js'
import { ref, reactive, onMounted } from 'vue'
import { ElMessageBox } from 'element-plus'
const formItem = [
  { comp: 'input', prop: 'userId', label: '用户ID', placeholder: '请输入用户ID' },
  {
    comp: 'select', prop: 'modeScreRange', label: '情绪评分', placeholder: '请选择评分范围', options: [
      { label: '低分: 1-3分', value: 1 },
      { label: '中分: 4-6分', value: 2 },
      { label: '高分: 7-10分', value: 3 },
    ]
  }
]
//列表数据
...
const handleSearch = async (formData) => {
  ...
  const { records, total } = await getEmotionalScore(params)
